...
class Report1Characteristic(Characteristic):
    ...
    def send(self):
        ...
        self.PropertiesChanged(GATT_CHRC_IFACE, { 'Value': [dbus.Byte(0x02),dbus.Byte(0x10)] }, [])	#first byte is modifer keys, second is keycode
    ...
    def StartNotify(self):
        print(f'Start Listening for interapplication messages')
        #GObject.timeout_add(10000, self.send)
        context = zmq.Context()
        socket = context.socket(zmq.REP)	#reply socket
        socket.bind("tcp://*:5555")			#listen for incoming message
        ...
        while True:
            ...
            parts = message.split("-")
            if (len(parts) == 7):
                modifyKeys = int(parts[0])
                keyCode0 = int(parts[1])
                keyCode1 = int(parts[1])
                keyCode2 = int(parts[1])
                keyCode3 = int(parts[1])
                keyCode4 = int(parts[1])
                keyCode5 = int(parts[1])
                #send via bluetooth:
                print(f'***sending keycode');
                #self.PropertiesChanged(GATT_CHRC_IFACE, { 'Value': [dbus.Byte(modifyKeys),dbus.Byte(keyCode0),dbus.Byte(keyCode1),dbus.Byte(keyCode2),dbus.Byte(keyCode3),dbus.Byte(keyCode4),dbus.Byte(keyCode5)] }, [])	#first byte is modifer keys, second is keycode
                self.PropertiesChanged(GATT_CHRC_IFACE, { 'Value': [dbus.Byte(modifyKeys),dbus.Byte(keyCode0)] }, [])	#first byte is modifer keys, second is keycode
                #automatically turn off key:
                #self.PropertiesChanged(GATT_CHRC_IFACE, { 'Value': [dbus.Byte(0x00),dbus.Byte(0x00),dbus.Byte(0x00),dbus.Byte(0x00),dbus.Byte(0x00),dbus.Byte(0x00),dbus.Byte(0x00)] }, [])
                print(f'***sent***')
            else:
            	print("Not enough digits in message")
            #reply
            socket.send_string("OK")	#must reply
